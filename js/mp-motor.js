...
import { effStat, consegueFugir, ordenarAcoes, freshVol, calcStats } from './regras.js';
...
export async function resolverTurnoMP(estado, acoes) {
  ...
  const ctx = { nome: m => m.nome, golpe: g => fmt(g.name), say, refDe: m => m.ref, monPorRef: r => monMP(s, r) };
  ...
  if (vivosMP(s.lados.A).length && vivosMP(s.lados.B).length) for (const m of vivosMP(todosMP(s))) await fimDeTurno(m, ctx); // + Speed Boost, Shed Skin
