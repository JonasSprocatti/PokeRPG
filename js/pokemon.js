...
// golpes = os 4 mais recentes aprendidos por nível. Usado pela criação (jogador) e pela batalha (selvagem).
import { STATS, NATURES } from './dados.js';
import { calcStats, freshVol, defaultMoves } from './regras.js';
...
export async function makeMon(data, level, opt = {}) {
  ...
  const mon = { id: data.id, name: data.name, nick: opt.nick || '', data, level, ivs, evs, nature, ability, moves, status: null, sleep: 0, exp: 0, vol: freshVol() };
